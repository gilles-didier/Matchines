...
double difftime_ms(struct timeval t1, struct timeval t0) {
    return ((double)(t1.tv_sec-t0.tv_sec))*1000. + (t1.tv_usec-t0.tv_usec)/1000.;
}
...
size_t *qsortindex(void *base, size_t nitems, size_t size, int (*compar)(const void *, const void*)) {
	...
	for(i=0; i<nitems; i++)
		p[i] = (void*)(((char*)data)+i*size);
	comparisonFunction = compar;
	qsort(p, nitems, sizeof(void*), compareIndirect);
	for(i=0; i<nitems; i++) {
		index[(((char*)p[i])-((char*)data))/size] = i;
		memcpy((void*)(((char*)base)+i*size), p[i], size);
	}
	monfree((void*)data);
	monfree((void*)p);
	return index;
